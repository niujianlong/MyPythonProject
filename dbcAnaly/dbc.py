...
fw = file("struct.h", "w+")
fw4 = file("missingCycleDefine.h", "w+")
fw5 = file("missingCycleTimeDefine.h", "w+")
fw6 = file("missingCycleArr.h", "w+")
fw7 = file("missingCycleCounter.h", "w+")
...
for line in open("ICM_ABUS_AND_VBUS_V1.2_20160525.txt"):  
    ...
    if line_split[0] == 'BO_':
        ...
        fw15.write('\nvoid Set'+frameStructName.lower()+'MissingDefaultValue()\n{\n')
        fw16.write('extern void Set'+frameStructName.lower()+'MissingDefaultValue();\n')
...
# getSigValue(Can_FrameType canRawData, UInt8 startByte, UInt8 startBit, UInt8 endBit, UInt8 bitLength,float resolution, UInt8 offset);
        ...
        for i in singleSigName:  # 打印宏定义供外界来调用
            ...
            for k in ibackup:
                if bitLengthList[a]<=8: 
                    fw9.write('  '+ frameStructName.lower()  +'.'+k+" = getuint8SigValue("+ frameStructName.lower()  +'.'+"data," + str(bitPragram[0+a*4]) + "," + str(bitPragram[1+a*4]) + "," + str(bitPragram[2+a*4]) +  ");\n")
                    fw9.write("  return " + frameStructName.lower()  +'.'+ k + ";\n}\n\n")
                elif bitLengthList[a]==16: 
                    fw9.write('  '+ frameStructName.lower()  +'.'+k+" = getuint16SigValue("+ frameStructName.lower()  +'.'+"data," + str(bitPragram[0+a*4]) + "," + str(bitPragram[3+a*4]) +  ");\n")
                    fw9.write("  return " + frameStructName.lower()  +'.'+ k + ";\n}\n\n")
                else: 
                    fw9.write('  '+ frameStructName.lower()  +'.'+k+" = getuint32SigValue("+ frameStructName.lower()  +'.'+"data," + str(bitPragram[0+a*4]) + "," + str(bitPragram[3+a*4]) +  ");\n")
                    fw9.write("  return " + frameStructName.lower()  +'.'+ k + ";\n}\n\n")
            a=a+1       
